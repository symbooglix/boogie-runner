...
def main(args):
    ...
    parser.add_argument("--remove-errors", dest='remove_errors', default=False, action='store_true', help='Remove error results as generated by boogie-batch-runner')
    ...
    # FIXME: We should make so that the relative order of the specified (by the user) command line options determines the order that filters are applied.
    # Setup filter functions
    filters = [ ]

    # Must remove errors first, if requested as other filters assume well formed results
    if pargs.remove_errors:
        filters.append(lambda r: 'error' not in r)
